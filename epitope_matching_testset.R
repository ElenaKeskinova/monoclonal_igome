...
graphs_min = lapply(names(dshort),\(i){
  require(igraph)
  e = dshort[[i]]
  edges = cbind(df_dist[[i]]$Residue1,df_dist[[i]]$Residue2)[e,]
  graph_from_edgelist(edges, directed = F)
})
names(graphs_min) = names(df_dist)
for(i in names(graphs_min)){plot(graphs_min[[i]],main = i)}
...
graphs_min = lapply(graphs_min,\(g){ # 1 letter code as attribute
...
allscores = future_lapply(names(mimotest_nC7),\(pdbname){
  scores = sapply(mimotest_nC7[[pdbname]],\(mimo){
    paths = pepmatch(pep = mimo,graphs_min[[pdbname]],zcorr)
...
neg_scores = future_lapply(names(mimotest_nC7),\(pdbname){
  scores = sapply(rnd_p,\(mimo){
    paths = pepmatch(pep = mimo,graphs_min[[pdbname]],corrmat)
    paths = merged_paths(paths,mimo,dist_mats[[pdbname]])
